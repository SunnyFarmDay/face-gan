...
# Find the most recent trial directory for ffhq_production_v3
TRIAL_DIR=$(ls -td trial_ffhq_production_v3* 2>/dev/null | head -1)
if [ -z "$TRIAL_DIR" ]; then
    CHECKPOINT_DIR=""
else
...
while true; do
    # Re-scan for trial directories and checkpoints on each iteration
    TRIAL_DIR=$(ls -td trial_ffhq_production_v3* 2>/dev/null | head -1)
    ...
    if [ -z "$LATEST_CHECKPOINT" ]; then
        for trial_dir in $(ls -td trial_ffhq_production_v3* 2>/dev/null); do
            checkpoint_dir="${trial_dir}/checkpoint"
            if [ -d "$checkpoint_dir" ]; then
